...
# dummy_input_embedding = construct_input_embeddings(batch["input_ids"].tolist()[0], token_embeddings)
# dummy_input_embedding = torch.reshape(dummy_input_embedding,
#                                       (1, dummy_input_embedding.size()[0], dummy_input_embedding.size()[1])).to(
...
dummy_labels = torch.randn((batch["input_ids"].size()[0], batch["input_ids"].size()[1], client.config.vocab_size)).to(
    DEVICE).requires_grad_(True)
# dummy_labels = torch.tensor(one_hot(batch["input_ids"].clone().detach(), num_classes=len(tokenizer)), dtype=torch.float32).to(DEVICE).requires_grad_(True)


optimizer = torch.optim.AdamW([dummy_input_embedding, dummy_labels], lr=0.01)
