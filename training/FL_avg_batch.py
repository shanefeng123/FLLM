...
while not early_stopping:
    ...
    for j in range(num_of_batches):
        ...
        if j % 10 == 0:
            for l in range(NUM_OF_CLIENTS):
                print(f"Round {i - 1}, client {l} average loss: {client_losses[l] / (j + 1)}")
                with open(f"{RESULTS_PATH}", "a") as file:
                    file.write(f"Round {i - 1}, client {l} average loss: {client_losses[l] / (j + 1)}\n")
